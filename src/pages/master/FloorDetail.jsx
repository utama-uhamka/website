...
const FloorDetail = () => {
  ...
  const loadItems = useCallback((page = itemPage) => {
    if (floorData) {
      dispatch(fetchItems({
        campus_id: floorData.building?.campus_id || floorData.campus_id,
  ...
  const roomColumns = [
    {
      ...
      render: (value) => (
        <img
          src={getRoomImageUrl(value) || logoFallback}
          alt="Room"
    ...
    { key: 'itemCount', label: 'Jumlah Item', width: '120px', render: (v) => v || 0 },
  ];
  ...
  // Room photo handler
  const handleRoomPhotoChange = async (e) => {
    ...
    try {
      ...
      setRoomPhotoFile(webpFile);
      setRoomPhotoPreview(URL.createObjectURL(webpFile));
    } catch (error) {
      toast.error('Gagal memproses gambar');
    }
  };

  const removeRoomPhoto = () => {
  ...
  const handleEditRoom = (item) => {
    ...
    setRoomPhotoFile(null);
    setRoomPhotoPreview(item.photo_1 ? getRoomImageUrl(item.photo_1) : '');
    setIsRoomModalOpen(true);
  };
  ...
  const handleRoomSubmit = async () => {
    ...
    try {
      ...
      if (roomPhotoFile) {
        ...
        submitData.append('room_name', roomForm.room_name);
        submitData.append('floor_id', id);
        submitData.append('photo_1', roomPhotoFile);
      } else {
        submitData = {
  ...
  const handleItemSubmit = async () => {
    ...
    try {
      ...
      submitData.append('campus_id', floorData.building?.campus_id || floorData.campus_id);
      submitData.append('building_id', floorData.building_id);
      submitData.append('floor_id', id);
  ...
  // Calculate stats
  const totalRooms = roomsPagination.total || roomsData.length;
    ...
    <MainLayout>
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: floorData.building?.campus?.campus_name || floorData.campus_name || 'Unit', path: `/master/units/${floorData.building?.campus_id || floorData.campus_id}` },
          { label: floorData.building?.building_name || floorData.building_name || 'Gedung', path: `/master/buildings/${floorData.building_id}` },
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-shrink-0">
            <div className="w-32 h-32 md:w-40 md:h-40 rounded-2xl overflow-hidden shadow-lg border-2 border-white">
              <img
                src={getFloorImageUrl(floorData.photo_1)}
                alt={floorData.floor_name}
          ...
          <div className="flex-1">
            <div className="grid grid-cols-2 md:grid-cols-2 gap-1">
              <Link
                to={`/master/units/${floorData.building?.campus_id || floorData.campus_id}`}
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="font-medium text-sm truncate">{floorData.building?.campus?.campus_name || floorData.campus_name || '-'}</p>
                </div>
              </Link>
              <Link
                to={`/master/buildings/${floorData.building_id}`}
                className="flex items-center gap-3 p-3 bg-white rounded-xl border border-gray-100 hover:border-primary/30 hover:shadow-sm transition-all"
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="font-medium text-sm truncate">{floorData.building?.building_name || floorData.building_name || '-'}</p>
                </div>
              </Link>
